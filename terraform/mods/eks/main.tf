...
locals {
  ...
  eks_default_addons = ["coredns","vpc-cni","kube-proxy"]
}
...
resource "aws_eks_addon" "this" {
  ...
  addon_version            = lookup(each.value, "addon_version", data.aws_eks_addon_version.eks_addons_recent_version[try(each.value.name, each.key)].version)
